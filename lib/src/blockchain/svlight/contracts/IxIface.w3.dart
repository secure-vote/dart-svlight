...
class IxIfaceContract {
  final ContractAbi $abi = ContractAbi.fromJson(IXIFACE_ABI, "IxIface");
  EthereumAddress $addr;
  DeployedContract $contract;
  Web3Client $client;
  IxIfaceContract(String address, Web3Client this.$client) {
    $addr = EthereumAddress.fromHex(address);
    $contract = DeployedContract($abi, $addr);
  }

  Future<String> dDeprecateCategory(Uint8List democHash, BigInt categoryId,
      Wallet wallet, TransactionNotPayable tx) async {
    var _f = $contract.function("dDeprecateCategory");
    var _params = _f.encodeCall(<dynamic>[democHash, categoryId]);
  ...
  Future<String> dInit(EthereumAddress defualtErc20,
      bool disableErc20OwnerClaim, Wallet wallet, TransactionPayable tx) async {
    var _f = $contract.function("dInit");
    var _params =
        _f.encodeCall(<dynamic>[defualtErc20, disableErc20OwnerClaim]);
    var finalized = tx.finalize(
        data: _params,
        from: await wallet.privateKey.extractAddress(),
        to: $contract.address);
