...
class _DrateaProgramState<TModel, TMsg, TSub>
  ...
  final StreamController<TMsg> _mainLoopController = StreamController();
  ...
  void initState() {
    ...
    var initialEffects = List<Effect<TMsg>>();
    ...
    if (widget.busStream != null) {
      final messagesBusStream = widget.busStream
          .transform<TMsg>(OfExactTypeStreamTransformer<TMsg>());
      msgStream =
    ...
    _appLoopSub = updStream
        .handleError((Exception e, StackTrace st) => program.onError(st, e))
        .listen((updates) {
