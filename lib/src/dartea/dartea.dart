...
class Program<TModel, TMsg, TSub> {
  ...
  Program(
    ...
    OnError onError,
  })  : onError = onError ?? ((s, e) => print('Dartea program error: $e\n$s')),
        sub = subscription ??
            emptySub<TModel, TMsg>() as Subscribe<TModel, TMsg, TSub>,
        lifeCycleUpdate = lifeCycleUpd ?? emptyLifecycleUpdate<TModel, TMsg>();

  ///Wrap all functions with [print]
  Program<TModel, TMsg, TSub> withDebugTrace() {
    return Program(
        () {
          print('Dartea program init');
          final res = init();
          print('Dartea program inited: ${res.model}');
          return res;
        },
        (msg, m) {
          print('Dartea handle message $msg. Current state is $m');
          final res = update(msg, m);
          print('Dartea updated state is ${res.model}');
          return res;
        },
        view,
        subscription: (s, d, m) {
          print('Dartea subscribe current sub:$s, model:$m');
          final res = sub(s, d, m);
          print('Dartea subscribe new sub:$res, model:$m');
          return res;
        },
        lifeCycleUpd: (s, m) {
          print('Dartea app state cahnged state:$s, current model:$m');
          final res = lifeCycleUpdate(s, m);
          print('Dartea app state cahnged state:$s, updated model:$m');
          return res;
        },
        onError: onError);
  }
...
///Ths widget creates and builds [Program].<T>
///T value;
///ValueKey(this.value);
