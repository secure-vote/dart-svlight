...
class BBFarmProxyContract {
  final ContractAbi $abi = ContractAbi.fromJson(BBFARMPROXY_ABI, "BBFarmProxy");
  EthereumAddress $addr;
  DeployedContract $contract;
  Web3Client $client;
  BBFarmProxyContract(String address, Web3Client this.$client) {
  ...
  Future<String> $constructor(Uint8List _namespace, Uint8List _foreignNetworkId,
      Wallet wallet, TransactionNotPayable tx) async {
    var _f = $abi.functions.where((f) => f.name == "" && f.isConstructor).first;
    var _params = _f.encodeCall(<dynamic>[_namespace, _foreignNetworkId]);
    var finalized = tx.finalize(
        data: _params,
        from: await wallet.privateKey.extractAddress(),
        to: $contract.address);
